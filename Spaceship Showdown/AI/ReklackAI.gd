	...
	match currenttask:
		...
		"Approach":
			...
			if currenttaskobj == null or distance(pos, currenttaskobj.position) < 240:
				...
				var anglediff = angle - (trajectory(1, currenttaskobj.position, currenttaskobj.velocity) - trajectory(1, pos, vel)).angle()
		...
		"Attack":
			framesidle = 0
			if currenttaskobj == null or batt < 7:
				print("Attack cancelled! " + str(currenttaskobj == null) +  " " + str(batt < 7) )
				set_task("Idle", null)
			else:
				var anglediff = angle - (trajectory(1, currenttaskobj.position, currenttaskobj.velocity) - trajectory(1, pos, vel)).angle()
				#var anglediff = angle_between(enemy.position, pos) - angle
